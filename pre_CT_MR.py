...
parser.add_argument(
    "-anatomy", type=str, default="Abd", help="Anaotmy name, [default: Abd]"
)
parser.add_argument(
    "-img_name_suffix",
    type=str,
    default="_0000.nii.gz",
...
parser.add_argument(
    "-img_path",
    type=str,
    default="data/FLARE22Train/images",
    help="Path to the nii images, [default: data/FLARE22Train/images]",
)
parser.add_argument(
    "-gt_path",
    type=str,
    default="data/FLARE22Train/labels",
...
parser.add_argument(
    "-window_level", type=int, default=40, help="CT window level, [default: 40]"
)
parser.add_argument(
    "-window_width", type=int, default=400, help="CT window width, [default: 400]"
)
parser.add_argument(
    "--save_nii",
    action="store_true",
...
def preprocess(name, npz_path):
    ...
    if len(z_index) > 0:
        ...
        np.savez_compressed(
            join(npz_path, prefix + gt_name.split(gt_name_suffix)[0] + ".npz"),
            imgs=img_roi,
            gts=gt_roi,
        ...
        if save_nii:
            ...
            sitk.WriteImage(
                img_roi_sitk,
                join(
                    npz_path, prefix + gt_name.split(gt_name_suffix)[0] + "_img.nii.gz"
                ),
            )
            ...
            sitk.WriteImage(
                gt_roi_sitk,
                join(
                    npz_path, prefix + gt_name.split(gt_name_suffix)[0] + "_gt.nii.gz"
                ),
            )
