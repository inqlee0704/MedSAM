...
parser.add_argument(
    ...
    default=True,
    type=bool,
    help="set it to False to disable NSD computation and save time",
)
        ...
        if dsc > 0.2:
            ...
            if npz_name.startswith("3D"):
                ...
                nsd = compute_multi_class_nsd(
                    np.expand_dims(gts, -1), np.expand_dims(segs, -1), spacing
                )
        else:
            nsd = 0.0
...
if __name__ == "__main__":
    ...
    with mp.Pool(num_workers) as pool:
        with tqdm(total=len(npz_names)) as pbar:
            for i, (npz_name, dsc, nsd) in enumerate(
                pool.imap_unordered(compute_metrics, npz_names)
            ):
