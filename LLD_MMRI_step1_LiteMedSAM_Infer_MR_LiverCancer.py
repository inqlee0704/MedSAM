...
def get_bbox(gt2D, bbox_shift=5):
    ...
    ), f"ground truth should be 0, 1, but got {np.unique(gt2D)}"
...
def infer_MR_liver_tumor(case_name):
    ...
    for case_phase in case_phases:
        ...
        bbox_2d = annotation["lesion"]["0"]["bbox"][
            "2D_box"
        ]  # list; each element is a dict: 'slice_idx', 'x_min', 'y_min', 'x_max', 'y_max', 'area'
        ...
        for bbox in bbox_2d:
            ...
            seg_mask = medsam_inference(
                medsam_lite_model, image_embedding, bboxes256, (newh, neww), (H, W)
            )
            # seg_mask = morphology.remove_small_holes(seg_mask>0)
            seg_data_3d[slice_idx, :, :] = np.uint8(
                morphology.remove_small_holes(seg_mask, 150)
            )
        ...
        sitk.WriteImage(
            seg_data_3d_sitk,
            join(save_dir, nii_name.replace("_0000.nii.gz", ".nii.gz")),
        )
...
if __name__ == "__main__":
    ...
    with mp.Pool(processes=num_workers) as pool:
        with tqdm(total=len(case_names)) as pbar:
            for i, _ in tqdm(
                enumerate(pool.imap_unordered(infer_MR_liver_tumor, case_names))
            ):
                pbar.update()
