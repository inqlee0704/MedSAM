...
class BboxPromptDemo:
    def __init__(self, model):
        ...
        self.x0, self.y0, self.x1, self.y1 = 0.0, 0.0, 0.0, 0.0
        self.rect = None
    ...
    def _infer(self, bbox):
        ...
        bbox_torch = (
            torch.as_tensor(bbox_1024, dtype=torch.float)
            .unsqueeze(0)
            .to(self.model.device)
        )
...
class PointPromptDemo:
    ...
    def infer(self, x, y):
        coords_1024 = np.array(
            [[[x * 1024 / self.img_size[1], y * 1024 / self.img_size[0]]]]
        )
        coords_torch = torch.tensor(coords_1024, dtype=torch.float32).to(
            self.model.device
        )
        ...
        low_res_pred = F.interpolate(
            low_res_probs, size=self.img_size, mode="bilinear", align_corners=False
        )
    ...
    def set_image(self, image_index):
        ...
        gt_resize = cv2.resize(
            gt, (self.img_size[1], self.img_size[0]), interpolation=cv2.INTER_NEAREST
        )
        self.gt = gt_resize
...
class TextPromptDemo:
    ...
    def infer(self, text):
        ...
        low_res_pred = torch.sigmoid(low_res_logits)  # (1, 1, 256, 256)
        low_res_pred = F.interpolate(
            low_res_pred, size=self.img_size, mode="bilinear", align_corners=False
        )
        low_res_pred = low_res_pred.detach().cpu().numpy().squeeze()
        seg = np.uint8(low_res_pred > 0.5)
    ...
    def show(self, fig_size=3, alpha=0.95):
        ...
        text = widgets.Text(
            value="", placeholder="Prompt", description="Prompt", disabled=False
        )
        display(text)
