...
parser.add_argument(
    "--save_overlay", action="store_true", help="whether to save the overlay image"
)
parser.add_argument(
    "-png_save_dir",
    type=str,
...
def MedSAM_infer_npz(gt_path_file):
    ...
    if (not isfile(join(pred_save_dir, task_folder, npz_name))) or overwrite:
        ...
        if gts.shape != (H, W):
            gts = cv2.resize(
                gts.astype(np.uint8), (W, H), interpolation=cv2.INTER_NEAREST
            ).astype(np.uint8)
        ...
        for label_id in label_ids:
            ...
            if gt2D.shape != (newh, neww):
                gt2D_resize = cv2.resize(
                    gt2D.astype(np.uint8), (neww, newh), interpolation=cv2.INTER_NEAREST
                ).astype(np.uint8)
            else:
            ...
            if np.sum(gt2D_padded) > 0:
                ...
                sam_mask = medsam_inference(
                    medsam_lite_model, image_embedding, box, (newh, neww), (H, W)
                )
        ...
        np.savez_compressed(
            join(pred_save_dir, task_folder, npz_name), segs=segs, gts=gts
        )
...
if __name__ == "__main__":
    ...
    with mp.Pool(processes=num_workers) as pool:
        with tqdm(total=len(gt_path_files)) as pbar:
            for i, _ in tqdm(
                enumerate(pool.imap_unordered(MedSAM_infer_npz, gt_path_files))
            ):
                pbar.update()
